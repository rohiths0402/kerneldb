...
Page *page_alloc(uint32_t page_id) {
    ...
    hdr->page_lsn = 0;
    hdr->slot_count = 0;
    hdr->free_start = PAGE_HEADER_SIZE;
...
PageResult page_insert(Page *page, const uint8_t *data, uint16_t length, uint16_t *slot_out) {
    PageHeader *hdr = get_header(page);
    uint16_t needed = sizeof(Slot) + length;
    if (free_space(page) < needed || hdr->slot_count >= MAX_SLOTS){
        return PAGE_FULL;
    }
    uint16_t row_offset = hdr->free_end - length;
    ...
    hdr->slot_count++;
    hdr->free_start = PAGE_HEADER_SIZE + hdr->slot_count * sizeof(Slot);
...
void page_compact(Page *page) {
    ...
    *tmp_hdr = *hdr;
    ...
    tmp_hdr->free_start = PAGE_HEADER_SIZE;
    tmp_hdr->free_end = PAGE_SIZE;
    ...
    for (uint16_t i = 0; i < hdr->slot_count; i++) {
        ...
        uint16_t newoff = tmp_hdr->free_end - len;
    ...
    tmp_hdr->free_start = PAGE_HEADER_SIZE + tmp_hdr->slot_count * sizeof(Slot);
    memcpy(page->data, tmp, PAGE_SIZE);
}

PageResult page_write_disk(const Page *page, int fd) {
    Page *mutable_page = (Page *)page;
    PageHeader *hdr = get_header(mutable_page);
    hdr->checksum = 0;
    hdr->checksum = compute_checksum(page);
...
void page_print(const Page *page) {
    const PageHeader *hdr   = get_header_c(page);
    const Slot *slots = get_slots_c(page);
    printf("\n  [page] id=%u  slots=%u  free=%u bytes  page_lsn=%llu\n", hdr->page_id, hdr->slot_count, (unsigned)(hdr->free_end - hdr->free_start), (unsigned long long)hdr->page_lsn);
    for (uint16_t i = 0; i < hdr->slot_count; i++) {
