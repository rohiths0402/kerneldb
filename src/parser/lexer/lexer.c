...
void lexer_tokenize(const char *input, TokenList *out) {
    ...
    while (*p) {
        ...
        if (*p == ';') { push(out, TOKEN_SEMICOLON, ";"); p++; continue; }
        if (*p == '(') { push(out, TOKEN_LPAREN,  "("); p++; continue; }
        if (*p == ')') { push(out, TOKEN_RPAREN,  ")"); p++; continue; }
        if (*p == ',') { push(out, TOKEN_COMMA,   ","); p++; continue; }
        if (*p == '*') { push(out, TOKEN_STAR,    "*"); p++; continue; }
        if (*p == '=') { push(out, TOKEN_EQUALS,  "="); p++; continue; }
        if (*p == '\'') {
            p++;
